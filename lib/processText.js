...
const latinCharacters = require("./latinCharaters");
const cirilicCharacters = require("./cirilicCharacters");
...
const calculateSpace = (font_size, total_lines, proportions) => {
	const space =
		font_size * total_lines +
		(font_size / proportions.line_height) * total_lines;
	return space;
};
...
const setFrases2Lines = (
	...
	createParams(lines, init_y, middle_x, font_size, [], proportions, function (
		params
	) {
		createParams(
			lines2,
...
const recursiveCalculateFont2Lines = (
	cont,
	text_length,
	font,
	proportions,
...
module.exports = {
	getParams: (paramsReq, callbackBot) => {
		...
		gm(paramsReq.path_img).size((err, size) => {
			if (!err) {
				processing(size);
			} else {
				...
				paramsReq.status.msg =
					"Upload failed! Please, try again later or choose another image.";
		...
		const processing = (size) => {
			...
			if (width < MIN_WIDTH || height < MIN_WIDTH) {
				if (
					...
					paramsReq.status.state = "error";
					paramsReq.status.msg =
						"Sorry, the image is too small for this text...";
					callbackBot(paramsReq);
					return;
				}
			}
		...
		const process1LineText = (atts, font_size, proportions, callback) => {
			...
			if (lines) {
				paramsReq.status = checkProportions(
					lines,
					[],
					atts,
					proportions,
