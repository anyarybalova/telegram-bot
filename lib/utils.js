...
module.exports = {
  ...
  getFilePath: (file_path, token, format, callbackFile) => {
    const url = url + token + '/' + file_path;
    const name = FOLDER_IMG + createNameFile();
    const path = name + format;
    let fileFormat = path;
    if (format === '.gif') {
      fileFormat = name + '.mp4';
    }
    let file = fs.createWriteStream(fileFormat);
    const request = https.get(url, response => {
      response.pipe(file);
      file.on('finish', () => {
        file.close();
        if (format === '.gif') {
          gify(fileFormat, path, err => {
            if (err) {
              console.log('Error en mp4');
              cleanFile(fileFormat);
            } else {
              callbackFile(path);
              cleanFile(path);
              cleanFile(fileFormat);
  ...
  getOptionMenuLines: () => {
    const options = {
      reply_markup: JSON.stringify({
        keyboard: [['2 lines - (bottom and top)'], ['1 line - top'], ['1 line - middle'], ['1 line - bottom']],
  ...
  getOptionFontColor: () => {
    const options = {
      reply_markup: JSON.stringify({
        keyboard: [['black', 'red'], ['blue', 'green'], ['white', 'yellow']],
        one_time_keyboard: true
      })
    };
    return options;
  ...
  getOptionFontType: () => {
    const options = {
      reply_markup: JSON.stringify({
        keyboard: [['meme font'], ['cursive'], ['normal']],
        one_time_keyboard: true
      })
    };
    return options;
  ...
  processImage: (params, callback) => {
    ...
    if (font_type === 'impact') {
      ...
      //without stroke
      myGm.fill(params.color).font(font, params.font_size);
    }
    ...
    const url = FOLDER_IMG + name + params.format;
    myGm.write(url, err => {
      if (!err) {
        console.log('done');
        setTimeout(() => {
