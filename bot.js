...
bot.on("photo", (msg) => {
	...
	const file_id = _.get(
		_.first(_.orderBy(photos, ["width"], ["desc"])),
		"file_id"
	);
...
const sendProcessedPhoto = (response, params) => {
	if (response.status === "ok") {
		if (params.format === ".jpg") {
			bot
				.sendPhoto(params.chat_id, fs.createReadStream(response.url))
				.then(() => {
			...
			sendMessageToBot(params.chat_id, messageChatId);
			bot
				.sendDocument(params.chat_id, fs.createReadStream(response.url))
				.then(() => {
